...
class NewAppointmentBookedNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $mail = (new MailMessage)
            ...
            ->when($this->appointment->attendee_phone, fn ($m) =>
                $m->line('Phone: ' . $this->appointment->attendee_phone)
            )
            ->when($this->appointment->notes, fn ($m) =>
                $m->line('Notes: ' . $this->appointment->notes)
            );
