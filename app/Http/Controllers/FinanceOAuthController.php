...
class FinanceOAuthController extends Controller
{
    ...
    protected function saveConnection(string $platform, array $tokenData): void
    {
        ...
        if ($platform === 'quickbooks' && isset($tokenData['realm_id'])) {
            $metadata['realmId'] = $tokenData['realm_id'];
        } elseif ($platform === 'xero' && session()->has('xero_tenant_id')) {
            $metadata['tenantId'] = session('xero_tenant_id');
            session()->forget('xero_tenant_id');
        } elseif ($platform === 'stripe' && isset($tokenData['stripe_user_id'])) {
            $metadata['stripe_user_id'] = $tokenData['stripe_user_id'];
        }
    ...
    /**
     * Fetch Xero account name and store tenantId
     */
    protected function fetchXeroAccountName(string $accessToken): ?string
    {
        ...
        if ($response->successful()) {
            ...
            // Store tenantId in session for saveConnection to pick up
